...
fn scroll_up_marks_all_rows() {
    ...
    screen.clear_dirty();
    screen.cursor.cursor.y = 2; // bottom row
...
fn test_write_char_range_dirty() {
    ...
    screen.clear_dirty();
    screen.cursor.cursor.x = 9; // last column
    screen.write_char('a');
    let ranges = screen.dirty_ranges();
    assert_eq!(
        ranges,
...
fn test_scroll_region_operations_damage() {
    ...
    screen.cursor.cursor.y = 0; // outside [1, 3]
    screen.clear_dirty();
    screen.insert_lines(2);
...
fn insert_lines_outside_region_noop() {
    ...
    screen.cursor.cursor.y = 0; // above scroll_top
    screen.insert_lines(1);
    assert_eq!(screen.row_text(0), "aaaa");
    assert_eq!(screen.row_text(1), "bbbb");
    assert_eq!(screen.row_text(2), "cccc");
...
fn delete_lines_outside_region_noop() {
    ...
    screen.cursor.cursor.y = 3; // below scroll_bottom
    screen.delete_lines(1);
    assert_eq!(screen.row_text(1), "bbbb");
    assert_eq!(screen.row_text(2), "cccc");
}
...
fn set_scroll_region_default() {
    let mut screen = Screen::new(4, 4);
    screen.cursor.scroll_region.top = 2;
    screen.cursor.scroll_region.bottom = 3;
    screen.set_scroll_region(0, 0);
    assert_eq!(screen.cursor.scroll_region.top, 0);
    assert_eq!(screen.cursor.scroll_region.bottom, 3);
...
fn set_scroll_region_custom() {
    ...
    assert_eq!(screen.cursor.scroll_region.top, 1);
    assert_eq!(screen.cursor.scroll_region.bottom, 2);
    assert_eq!(screen.cursor.cursor.x, 0);
    assert_eq!(screen.cursor.cursor.y, 0);
}
...
fn newline_below_region_no_panic() {
    ...
    screen.cursor.scroll_region.top = 1;
    screen.cursor.scroll_region.bottom = 2;
    screen.cursor.cursor.y = 3; // below region at last row
    screen.cursor.cursor.x = 0;
    screen.newline(); // must NOT panic
    // Cursor should not have wrapped past rows-1.
    assert!(screen.cursor.cursor.y < screen.rows());
    assert_eq!(screen.cursor.cursor.y, 3);
    assert_eq!(screen.cursor.cursor.x, 0);
}
...
fn resize_preserves_saved_cursor() {
    ...
    screen.restore_cursor();
    assert_eq!(screen.cursor.cursor.x, 3, "original x preserved");
...
fn test_autowrap_and_pending_wrap() {
    ...
    screen.cursor.modes.autowrap = true;
...
fn test_cursor_visibility() {
    ...
    assert!(screen.cursor.cursor.visible);
...
fn set_scroll_region_homes_cursor_within_origin_mode() {
    ...
    screen.cursor.margins.right = 6;
    screen.cursor.cursor.x = 5;
    screen.cursor.cursor.y = 5;

    screen.set_scroll_region(2, 5);

    assert_eq!((screen.cursor.cursor.x, screen.cursor.cursor.y), (2, 1));
}
...
fn test_horizontal_margins() {
    ...
    screen.cursor.margins.enabled = true;
    screen.cursor.cursor.x = 1; // pointing at '2'
...
fn test_tab_stops_hts_tbc() {
    ...
    assert_eq!(screen.cursor.cursor.x, 8);
    ...
    screen.cursor.cursor.x = 0;
    screen.horizontal_tab();
    assert_eq!(screen.cursor.cursor.x, 4, "should jump to custom tab stop");
...
fn line_feed_mode_defaults_and_resets_disabled() {
    ...
    assert_eq!(screen.cursor.cursor.x, 3);
    assert_eq!(screen.cursor.cursor.y, 4);

    parser.put_bytes(&mut screen, b"\x1b[20h\x1bc");
    assert!(!screen.cursor.modes.line_feed);
...
fn index_is_independent_of_line_feed_mode() {
    ...
    assert!(screen.cursor.modes.line_feed);
    assert_eq!((screen.cursor.cursor.x, screen.cursor.cursor.y), (4, 2));
}
...
fn test_decslrm_csi_dispatch() {
    ...
    assert_eq!(screen.cursor.margins.left, 1);
    assert_eq!(screen.cursor.margins.right, 3);
}
...
fn reversed_rectangular_ranges_are_ignored() {
    ...
    for origin_mode in [false, true] {
        for (name, sequence) in invalid_sequences {
            ...
            if origin_mode {
                screen.cursor.scroll_region.top = 1;
                screen.cursor.scroll_region.bottom = 4;
                screen.cursor.margins.left = 1;
                screen.cursor.margins.right = 4;
                screen.cursor.modes.origin = true;
            }
...
fn test_margin_wide_character_wrapping() {
    ...
    screen2.cursor.margins.enabled = true;
    screen2.cursor.margins.left = 2;
    screen2.cursor.margins.right = 5;
    screen2.cursor.modes.autowrap = true;
    screen2.cursor.cursor.x = 5;
    screen2.cursor.cursor.y = 0;
    ...
    screen3.cursor.margins.enabled = true;
    screen3.cursor.margins.left = 2;
    screen3.cursor.margins.right = 5;
    screen3.cursor.modes.autowrap = false;
    screen3.cursor.cursor.x = 5;
    screen3.write_char('中');
    assert_eq!((screen3.cursor.cursor.x, screen3.cursor.cursor.y), (5, 0));
...
fn alt_screen_restores_all_state_groups() {
    ...
    // CursorState
    assert_eq!(screen.cursor.cursor.x, 5, "cursor.x restored");
    assert_eq!(screen.cursor.cursor.y, 2, "cursor.y restored");
    assert_eq!(
        screen.cursor.cursor.shape,
        CursorShape::Block,
        "cursor shape restored"
    );
    assert!(!screen.cursor.cursor.blink, "cursor blink restored");
    ...
    // Margins
    assert!(screen.cursor.margins.enabled, "margin mode restored");
    assert_eq!(screen.cursor.margins.left, 2, "margin left restored");
    assert_eq!(screen.cursor.margins.right, 9, "margin right restored");
    // TerminalModes
    assert!(!screen.cursor.modes.autowrap, "autowrap restored");
    assert!(!screen.cursor.modes.pending_wrap, "pending wrap restored");
    assert!(screen.cursor.modes.origin, "origin mode restored");
    assert!(screen.cursor.modes.insert, "insert mode restored");
    assert!(screen.cursor.modes.line_feed, "line feed mode restored");
    assert!(
        screen.cursor.modes.application_cursor,
        "application cursor restored"
    );
    assert!(
        screen.cursor.modes.application_keypad,
    ...
    screen.cursor.cursor.y = 0;
    screen.restore_cursor();
    assert_eq!(screen.cursor.cursor.x, 5, "saved cursor x restored");
    assert_eq!(screen.cursor.cursor.y, 2, "saved cursor y restored");
}
