...
## Terminal Grid Domain

### Selection
- **Definition:** A range of highlighted cells in the terminal grid, defined by an anchor and cursor in generation-coordinate space. The range is rendered as a colored overlay and can be copied via Ctrl+C.
- **Synonyms:** Highlight, mark, region
- **Relationships:**
  - belongs to Screen

### Selection cancellation
- **Definition:** The removal of an existing selection after a keyboard action, including scrollback navigation.
- **Synonyms:** Deselect, selection reset

### SelectionGranularity
- **Definition:** The semantic unit by which a selection expands: `Character` (free drag), `Word` (double-click + word-wise drag), or `Line` (triple-click + line-wise drag).
- **Synonyms:** Level, mode

### Anchor
- **Definition:** The fixed endpoint of a selection range, set at the initial click position.
- **Synonyms:** Start point, origin

### Cursor (selection context)
- **Definition:** The movable endpoint of a selection range that tracks the mouse during drag.
- **Synonyms:** End point, drag point

### Click chain
- **Definition:** A sequence of `MouseInput::Pressed` events within the multi-click timeout (500ms) at the same grid cell. Click count determines selection granularity: 1 = Character, 2 = Word, 3+ = Line.
- **Synonyms:** Multi-click sequence

### Multi-click timeout
- **Definition:** The maximum interval (500ms) between consecutive clicks for them to count as a click chain.
- **Synonyms:** Double-click speed, click delay

### Word boundary
- **Definition:** A position in a row where a word starts or ends, determined by the `WORD_SEPARATORS` set plus CJK character categories. For initial word finding (double-click), CJK characters group together but stop at separators or non-CJK word characters; for word-wise drag, each CJK character is its own boundary.
- **Synonyms:** Word delimiter

### CJK character
- **Definition:** A character in one of these Unicode blocks: CJK Unified Ideographs (U+4E00–U+9FFF and extensions), Hiragana (U+3040–U+309F), Katakana (U+30A0–U+30FF), and Hangul Syllables (U+AC00–U+D7AF). These characters participate in word selection with special grouping-vs-drag semantics.
- **Synonyms:** Ideograph

### Word-wise drag
- **Definition:** Drag behavior after a double-click; the cursor snaps to word boundaries as it moves across rows. For CJK text this means each CJK character is an individual snap point, allowing character-by-character expansion.
- **Synonyms:** Smart selection, semantic drag

### Line-wise drag
- **Definition:** Drag behavior after a triple-click; the cursor snaps to the first/last column of each row as it moves.
- **Synonyms:** Line drag, row drag

### Scrollback
- **Definition:** Retained primary-screen terminal output that precedes the live viewport and remains available for review.
- **Synonyms:** Terminal history

### Scrollback navigation
- **Definition:** User-initiated movement of the viewport through scrollback. In the normal screen, PageUp, PageDown, Home, and End belong to Harbor rather than the PTY; the alternate screen retains application ownership of those keys.
- **Synonyms:** Paging, history navigation

### Scrollback page
- **Definition:** One current viewport height, measured in terminal rows. PageUp and PageDown move by one scrollback page.
- **Synonyms:** Fixed page size

### TerminalReply
- **Definition:** A platform-neutral byte buffer in the Screen struct storing outgoing terminal protocol replies generated by the VT parser.
